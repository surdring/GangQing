...
- **本地开发 `.env.local`（强制）**：
  - **加载策略**：本地开发与测试默认从仓库根目录 `.env.local` 加载配置（如存在）。
  - **优先级**：配置读取优先级必须为 **进程环境变量** > **`.env.local`**，以便 CI/容器/生产通过环境变量覆盖本地默认值。
  - **禁止交互式询问**：服务/脚本运行时不得通过交互式提问获取关键配置；关键配置缺失必须快速失败，并输出清晰的英文错误消息（例如缺少 `GANGQING_DATABASE_URL`）。
  - **文件边界**：`.env.local` 仅用于本地开发与测试，不得提交到仓库；`.env.example` 必须完整枚举所有配置项并说明用途。
...
#### 4.4.1 数据库 RLS（Row Level Security）决策与前置条件（L2+ 可选）

- **决策（L1）**：L1 阶段不将 PostgreSQL RLS 作为硬依赖（主策略为应用层默认过滤 + 跨域拒绝 + 可审计）。
- **定位（L2+）**：RLS 可作为第二道防线（Defense-in-Depth）可选启用；启用后建议结合 `FORCE ROW LEVEL SECURITY`，并避免使用具备 `BYPASSRLS` 的数据库角色执行应用查询。
- **前置条件（已具备）**：当前应用在审计读写与部分查询链路已写入会话变量（如 `set_config('app.current_tenant', ...)` / `set_config('app.current_project', ...)`），该机制可作为未来 RLS policy 的上下文输入。
